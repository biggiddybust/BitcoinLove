...
public:
    ...
    std::string operator()(const libbitcoinlove::SproutPaymentAddress& zaddr) const
    {
    ...
    std::string operator()(const libbitcoinlove::SaplingPaymentAddress& zaddr) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zaddr;
    ...
    std::string operator()(const libbitcoinlove::InvalidEncoding& no) const { return {}; }
};
...
public:
    ...
    std::string operator()(const libbitcoinlove::SproutViewingKey& vk) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << vk;
...
public:
    ...
    std::string operator()(const libbitcoinlove::SproutSpendingKey& zkey) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zkey;
    ...
    std::string operator()(const libbitcoinlove::SaplingExtendedSpendingKey& zkey) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zkey;
        // ConvertBits requires unsigned char, but CDataStream uses char
...
std::string EncodePaymentAddress(const libbitcoinlove::PaymentAddress& zaddr)
{
    return boost::apply_visitor(PaymentAddressEncoder(Params()), zaddr);
}

libbitcoinlove::PaymentAddress DecodePaymentAddress(const std::string& str)
{
    std::vector<unsigned char> data;
    if (DecodeBase58Check(str, data)) {
        const std::vector<unsigned char>& zaddr_prefix = Params().Base58Prefix(CChainParams::ZCPAYMENT_ADDRRESS);
        if ((data.size() == libbitcoinlove::SerializedSproutPaymentAddressSize + zaddr_prefix.size()) &&
            ...
            libbitcoinlove::SproutPaymentAddress ret;
            ss >> ret;
            return ret;
        }
    }
    ...
    if (bech.first == Params().Bech32HRP(CChainParams::SAPLING_PAYMENT_ADDRESS) &&
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin(), bech.second.end())) {
            ...
            libbitcoinlove::SaplingPaymentAddress ret;
            ss >> ret;
            return ret;
        }
    }
    return libbitcoinlove::InvalidEncoding();
}
...
libbitcoinlove::SpendingKey DecodeSpendingKey(const std::string& str)
{
    ...
    if (bech.first == Params().Bech32HRP(CChainParams::SAPLING_EXTENDED_SPEND_KEY) &&
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin(), bech.second.end())) {
            CDataStream ss(data, SER_NETWORK, PROTOCOL_VERSION);
            libbitcoinlove::SaplingExtendedSpendingKey ret;
            ss >> ret;
            memory_cleanse(data.data(), data.size());
            return ret;
        }
