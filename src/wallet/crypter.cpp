                               ...
                               libbitcoinlove::SproutSpendingKey& sk)
{
    ...
    if (vchSecret.size() != libbitcoinlove::SerializedSproutSpendingKeySize)
        return false;
...
static bool DecryptSaplingSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libbitcoinlove::SaplingExtendedSpendingKey& sk)
{
    CKeyingMaterial vchSecret;
...
{
    {
        ...
        {
            const libbitcoinlove::SproutPaymentAddress &address = (*miSprout).first;
        ...
        {
            const libbitcoinlove::SaplingExtendedFullViewingKey &extfvk = (*miSapling).first;
...
bool CCryptoKeyStore::AddSproutSpendingKey(const libbitcoinlove::SproutSpendingKey &sk)
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
bool CCryptoKeyStore::GetSproutSpendingKey(const libbitcoinlove::SproutPaymentAddress &address, libbitcoinlove::SproutSpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
bool CCryptoKeyStore::GetSaplingSpendingKey(const libbitcoinlove::SaplingFullViewingKey &fvk, libbitcoinlove::SaplingExtendedSpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libbitcoinlove::SproutSpendingKey &sk = mSproutSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libbitcoinlove::SproutPaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
