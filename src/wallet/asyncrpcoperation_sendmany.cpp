...
#include "utiltime.h"
#include "bitcoinlove/IncrementalMerkleTree.hpp"
#include "sodium.h"
#include "miner.h"
...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    if (isUsingBuilder_) {
        ...
        uint256 ovk;
        if (isfromzaddr_) {
            auto sk = boost::get<libbitcoinlove::SaplingExtendedSpendingKey>(spendingkey_);
            expsk = sk.expsk;
        ...
        for (auto r : z_outputs_) {
            ...
            assert(boost::get<libbitcoinlove::SaplingPaymentAddress>(&addr) != nullptr);
            auto to = boost::get<libbitcoinlove::SaplingPaymentAddress>(addr);
    ...
    if (isfromtaddr_) {
        ...
        while (zOutputsDeque.size() > 0) {
            ...
            while (n++<ZC_NUM_JS_OUTPUTS && zOutputsDeque.size() > 0) {
                ...
                JSOutput jso = JSOutput(boost::get<libbitcoinlove::SproutPaymentAddress>(pa), value);
                if (hexMemo.size() > 0) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            ZCNoteDecryption decryptor(boost::get<libbitcoinlove::SproutSpendingKey>(spendingkey_).receiving_key());
            auto hSig = prevJoinSplit.h_sig(*pbitcoinloveParams, tx_.joinSplitPubKey);
            ...
            try {
                ...
                SproutNote note = plaintext.note(boost::get<libbitcoinlove::SproutPaymentAddress>(frompaymentaddress_));
        ...
        if (jsChange>0) {
            info.vjsout.push_back(JSOutput(boost::get<libbitcoinlove::SproutPaymentAddress>(frompaymentaddress_), jsChange));
        ...
        uint256 anchor)
{
    ...
    for (size_t i = 0; i < witnesses.size(); i++) {
        ...
        info.vjsin.push_back(JSInput(*witnesses[i], info.notes[i], boost::get<libbitcoinlove::SproutSpendingKey>(spendingkey_)));
    }
    ...
    std::array<libbitcoinlove::JSInput, ZC_NUM_JS_INPUTS> inputs
            {info.vjsin[0], info.vjsin[1]};
    std::array<libbitcoinlove::JSOutput, ZC_NUM_JS_OUTPUTS> outputs
            ...
            &esk); // parameter expects pointer to esk, so pass in address
    {
        ...
        if (!(jsdesc.Verify(*pbitcoinloveParams, verifier, joinSplitPubKey_))) {
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pbitcoinloveParams, joinSplitPubKey_);
    ...
    for (int i = 0; i < ZC_NUM_JS_OUTPUTS; i++) {
        ...
        libbitcoinlove::SproutPaymentAddress zaddr = output.addr;  // randomized output
