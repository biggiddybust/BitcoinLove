...
CWalletTx GetValidSproutReceive(const libbitcoinlove::SproutSpendingKey& sk, CAmount value, bool randomInputs, int32_t version = 2) {
    return GetValidSproutReceive(*params, sk, value, randomInputs, version);
}

CWalletTx GetInvalidCommitmentSproutReceive(const libbitcoinlove::SproutSpendingKey& sk, CAmount value, bool randomInputs, int32_t version = 2) {
    return GetInvalidCommitmentSproutReceive(*params, sk, value, randomInputs, version);
}

libbitcoinlove::SproutNote GetSproutNote(const libbitcoinlove::SproutSpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetSproutNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSproutSpend(const libbitcoinlove::SproutSpendingKey& sk,
                        const libbitcoinlove::SproutNote& note, CAmount value) {
...
TEST(WalletTests, SetSaplingNoteAddrsInCWalletTx) {
    ...
    libbitcoinlove::SaplingNote note(pk, 50000);
    auto cm = note.cm().get();
...
TEST(WalletTests, CheckSproutNoteCommitmentAgainstNotePlaintext) {
    ...
    ASSERT_THROW(wallet.GetSproutNoteNullifier(
        ...
        address,
        dec,
        hSig, 1), libbitcoinlove::note_decryption_failed);
}
...
TEST(WalletTests, GetConflictedSaplingNotes) {
    ...
    auto maybe_pt = libbitcoinlove::SaplingNotePlaintext::decrypt(
...
TEST(WalletTests, SetBestChainIgnoresTxsWithoutShieldedData) {
    ...
    mtxSapling.vShieldedOutput[0].cv = libbitcoinlove::random_uint256();
    ...
    mtxSaplingTransparent.vShieldedOutput[0].cv = libbitcoinlove::random_uint256();
...
TEST(WalletTests, MarkAffectedSaplingTransactionsDirty) {
    ...
    // 0.0005 t-BLV in, 0.0004 z-BLV out, 0.0001 t-BLV fee
    ...
    // 0.0004 z-BLV in, 0.00025 z-BLV out, 0.0001 t-BLV fee, 0.00005 z-BLV change
    auto builder2 = TransactionBuilder(consensusParams, 2, expiryDelta);
