#!/usr/bin/env python2
#
# Execute all of the automated tests related to Bitcoinlove.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/bitcoinlove/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/bitcoinlove/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if bitcoinloved is an ELF binary, they all are
    with open(repofile('src/bitcoinloved'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/bitcoinloved')
    ret &= test_rpath_runpath('src/bitcoinlove-cli')
    ret &= test_rpath_runpath('src/bitcoinlove-gtest')
    ret &= test_rpath_runpath('src/bitcoinlove-tx')
    ...
    ret &= test_fortify_source('src/bitcoinloved')
    ret &= test_fortify_source('src/bitcoinlove-cli')
    ret &= test_fortify_source('src/bitcoinlove-gtest')
    ret &= test_fortify_source('src/bitcoinlove-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/bitcoinlove-gtest')],
