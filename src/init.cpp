...
ZCJoinSplit* pbitcoinloveParams = NULL;
...
void Shutdown()
{
    ...
    pwalletMain = NULL;
#endif
    delete pbitcoinloveParams;
    pbitcoinloveParams = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bitcoinlove.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bitcoinloved.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16525, 26525));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 16524, 26524));
...
)
{
    ...
    if (!(
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Bitcoinlove network parameters in the following directory:\n"
              "%s\n"
    ...
    pbitcoinloveParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-migrationdestaddress")) {
        ...
        libbitcoinlove::PaymentAddress address = DecodePaymentAddress(migrationDestAddress);
        if (boost::get<libbitcoinlove::SaplingPaymentAddress>(&address) == nullptr) {
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitcoinlove is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitcoinlove is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitcoinlove is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize Bitcoinlove circuit parameters
    ...
    if (GetBoolArg("-savesproutr1cs", false)) {
        ...
        pbitcoinloveParams->saveR1CS(r1cs_path.string());
    }
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Bitcoinlove was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Bitcoinlove with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Bitcoinlove was not built with wallet support. Set -mineraddress, or rebuild Bitcoinlove with wallet support."));
    }
 #endif // !ENABLE_WALLET
