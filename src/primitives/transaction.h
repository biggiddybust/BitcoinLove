...
#include "bitcoinlove/Bitcoinlove.h"
...
public:
    ...
    libbitcoinlove::GrothProof zkproof;  //!< A zero-knowledge proof using the spend circuit.
...
public:
    ...
    libbitcoinlove::SaplingEncCiphertext encCiphertext; //!< A ciphertext component for the encrypted output note.
    libbitcoinlove::SaplingOutCiphertext outCiphertext; //!< A ciphertext component for the encrypted output note.
    libbitcoinlove::GrothProof zkproof;   //!< A zero-knowledge proof using the output circuit.
...
public:
    ...
    void operator()(const libbitcoinlove::PHGRProof& proof) const
    {
        if (useGroth) {
    ...
    void operator()(const libbitcoinlove::GrothProof& proof) const
    {
        if (!useGroth) {
...
{
    if (useGroth) {
        libbitcoinlove::GrothProof grothProof;
        ::Unserialize(s, grothProof);
        proof = grothProof;
    } else {
        libbitcoinlove::PHGRProof pghrProof;
...
public:
    ...
    libbitcoinlove::SproutProof proof;
    ...
    JSDescription(
            ...
            const uint256& rt,
            const std::array<libbitcoinlove::JSInput, ZC_NUM_JS_INPUTS>& inputs,
            const std::array<libbitcoinlove::JSOutput, ZC_NUM_JS_OUTPUTS>& outputs,
            CAmount vpub_old,
            CAmount vpub_new,
    ...
    bool Verify(
        ZCJoinSplit& params,
        libbitcoinlove::ProofVerifier& verifier,
        const uint256& joinSplitPubKey
    ) const;
