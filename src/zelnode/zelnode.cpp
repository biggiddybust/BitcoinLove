...
void Zelnode::Check(bool forceCheck)
{
    ...
    if (!forceCheck && (GetTime() - lastTimeChecked < BLVNODE_CHECK_SECONDS)) return;
    lastTimeChecked = GetTime();


    //once spent, stop doing the checks
    if (activeState == BLVNODE_VIN_SPENT) return;


    if (!IsPingedWithin(BLVNODE_REMOVAL_SECONDS)) {
        activeState = BLVNODE_REMOVE;
        return;
    }

    if (!IsPingedWithin(BLVNODE_EXPIRATION_SECONDS)) {
        activeState = BLVNODE_EXPIRED;
        return;
    }

    if(lastPing.sigTime - sigTime < BLVNODE_MIN_ZNP_SECONDS){
        activeState = BLVNODE_PRE_ENABLED;
        return;
    }
    ...
    if (!unitTest) {
...

        {
            ...
            if (!AcceptableInputs(mempool, state, CTransaction(tx), false, NULL)) {
                activeState = BLVNODE_VIN_SPENT;
                return;
            }
        }
    }

    activeState = BLVNODE_ENABLED; // OK
}
...
bool ZelnodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (!obfuScationSigner.VerifyMessage(pubKeyCollateralAddress, sig, GetStrMessage(), errorMessage))
    {
        ...
        nDos = protocolVersion < MIN_PEER_PROTO_VERSION_BLVNODE ? 0 : 100;
        return error("%s - Got bad Zelnode address signature : %s", __func__, errorMessage);
    }

    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 16525) return false;
    } else if (addr.GetPort() == 16525)
        return false;
    ...
    if (pzn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pzn->IsBroadcastedWithin(BLVNODE_MIN_ZNB_SECONDS)) {
...
bool ZelnodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (GetInputAge(vin) < BLVNODE_MIN_CONFIRMATIONS) {
        LogPrint("zelnode","znb - Input must have at least %d confirmations\n", BLVNODE_MIN_CONFIRMATIONS);
    ...
    // should be at least not earlier than block when 10000, 25000, 100000 BLV tx got BLVNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = uint256();
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pZNIndex = (*mi).second;                                                        // block for 10000, 25000, 100000 BLV tx -> 1 confirmation
        CBlockIndex* pConfIndex = chainActive[pZNIndex->nHeight + BLVNODE_MIN_CONFIRMATIONS - 1]; // block where tx got BLVNODE_MIN_CONFIRMATIONS
        if (pConfIndex->GetBlockTime() > sigTime) {
            LogPrint("zelnode","znb - Bad sigTime %d for Zelnode %s (%i conf block is at %d)\n",
                     sigTime, vin.prevout.hash.ToString(), BLVNODE_MIN_CONFIRMATIONS, pConfIndex->GetBlockTime());
            return false;
        }
    }
...
{
    CInv inv(MSG_BLVNODE_ANNOUNCE, GetHash());
    RelayInv(inv);
}
...
bool ZelnodePing::CheckAndUpdate(int& nDos, bool fRequireEnabled, bool fCheckSigTimeOnly)
{
    ...
    if (pzn != NULL && pzn->protocolVersion >= zelnodePayments.GetMinZelnodePaymentsProto()) {
        ...
        // last ping was more then BLVNODE_MIN_MNP_SECONDS-60 ago comparing to this one
        if (!pzn->IsPingedWithin(BLVNODE_MIN_ZNP_SECONDS - 60, sigTime)) {
...
{
    CInv inv(MSG_BLVNODE_PING, GetHash());
    RelayInv(inv);
}
