...
TEST(keystore_tests, sapling_keys) {
    ...
    for (size_t i = 2; i < 12; i++) {
        ...
        libbitcoinlove::diversifier_t default_d;
        ...
        auto sk = libbitcoinlove::SaplingSpendingKey(skSeed);
        
        // Check that expanded spending key from primitives and from sk are the same
        auto exp_sk_2 = libbitcoinlove::SaplingExpandedSpendingKey(ask, nsk, ovk);
        ...
        auto full_viewing_key_2 = libbitcoinlove::SaplingFullViewingKey(ak, nk, ovk);
        ...
        auto in_viewing_key_2 = libbitcoinlove::SaplingIncomingViewingKey(ivk);
        ...
        auto default_addr_3 = libbitcoinlove::SaplingPaymentAddress(default_d, default_pk_d);
...
TEST(keystore_tests, store_and_retrieve_spending_key) {
    CBasicKeyStore keyStore;
    libbitcoinlove::SproutSpendingKey skOut;

    std::set<libbitcoinlove::SproutPaymentAddress> addrs;
    keyStore.GetSproutPaymentAddresses(addrs);
    EXPECT_EQ(0, addrs.size());

    auto sk = libbitcoinlove::SproutSpendingKey::random();
...
TEST(keystore_tests, StoreAndRetrieveViewingKey) {
    CBasicKeyStore keyStore;
    libbitcoinlove::SproutViewingKey vkOut;
    libbitcoinlove::SproutSpendingKey skOut;
    ZCNoteDecryption decOut;
...
TEST(keystore_tests, StoreAndRetrieveSaplingSpendingKey) {
    CBasicKeyStore keyStore;
    libbitcoinlove::SaplingExtendedSpendingKey skOut;
    libbitcoinlove::SaplingFullViewingKey fvkOut;
    libbitcoinlove::SaplingIncomingViewingKey ivkOut;

    std::vector<unsigned char, secure_allocator<unsigned char>> rawSeed(32);
    HDSeed seed(rawSeed);
    auto sk = libbitcoinlove::SaplingExtendedSpendingKey::Master(seed);
...
TEST(keystore_tests, StoreAndRetrieveHDSeedInEncryptedStore) {
    ...
    ASSERT_TRUE(keyStore2.AddSproutSpendingKey(libbitcoinlove::SproutSpendingKey::random()));
...
TEST(keystore_tests, store_and_retrieve_spending_key_in_encrypted_store) {
    ...
    libbitcoinlove::SproutSpendingKey keyOut;
    ZCNoteDecryption decOut;
    std::set<libbitcoinlove::SproutPaymentAddress> addrs;
    ...
    auto sk2 = libbitcoinlove::SproutSpendingKey::random();
