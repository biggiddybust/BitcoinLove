...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 joinSplitPubKey;
    auto verifier = libbitcoinlove::ProofVerifier::Strict();
    joinsplit.Verify(*pbitcoinloveParams, verifier, joinSplitPubKey);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_sprout_notes(size_t nKeys)
{
    ...
    auto sk = libbitcoinlove::SproutSpendingKey::random();
    auto tx = GetValidSproutReceive(*pbitcoinloveParams, sk, 10, true);
...
CWalletTx CreateSproutTxWithNoteData(const libbitcoinlove::SproutSpendingKey& sk) {
    auto wtx = GetValidSproutReceive(*pbitcoinloveParams, sk, 10, true);
    auto note = GetSproutNote(*pbitcoinloveParams, sk, wtx, 0, 1);
...
double benchmark_increment_sprout_note_witnesses(size_t nTxs)
{
    ...
    auto sproutSpendingKey = libbitcoinlove::SproutSpendingKey::random();
...
double benchmark_create_sapling_output()
{
    ...
    libbitcoinlove::SaplingNotePlaintext notePlaintext(note, memo);
