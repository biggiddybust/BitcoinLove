...
CMutableTransaction GetValidSproutReceiveTransaction(ZCJoinSplit& params,
    ...
    std::array<libbitcoinlove::JSInput, 2> inputs = {
        libbitcoinlove::JSInput(), // dummy input
        libbitcoinlove::JSInput() // dummy input
    };

    std::array<libbitcoinlove::JSOutput, 2> outputs = {
        libbitcoinlove::JSOutput(sk.address(), value),
        libbitcoinlove::JSOutput(sk.address(), value)
    };
...
CWalletTx GetValidSproutSpend(ZCJoinSplit& params,
    ...
    libbitcoinlove::JSOutput dummyout;
    libbitcoinlove::JSInput dummyin;

    {
        if (note.value() > value) {
            libbitcoinlove::SproutSpendingKey dummykey = libbitcoinlove::SproutSpendingKey::random();
            libbitcoinlove::SproutPaymentAddress dummyaddr = dummykey.address();
            dummyout = libbitcoinlove::JSOutput(dummyaddr, note.value() - value);
        } else if (note.value() < value) {
            libbitcoinlove::SproutSpendingKey dummykey = libbitcoinlove::SproutSpendingKey::random();
            libbitcoinlove::SproutPaymentAddress dummyaddr = dummykey.address();
            libbitcoinlove::SproutNote dummynote(dummyaddr.a_pk, (value - note.value()), uint256(), uint256());
            tree.append(dummynote.cm());
            dummyin = libbitcoinlove::JSInput(tree.witness(), dummynote, dummykey);
        }
    }
    ...
    std::array<libbitcoinlove::JSInput, 2> inputs = {
        libbitcoinlove::JSInput(tree.witness(), note, sk),
        dummyin
    };

    std::array<libbitcoinlove::JSOutput, 2> outputs = {
        dummyout, // dummy output
        libbitcoinlove::JSOutput() // dummy output
    };
...
libbitcoinlove::SaplingExtendedSpendingKey GetTestMasterSaplingSpendingKey() {
    ...
    HDSeed seed(rawSeed);
    return libbitcoinlove::SaplingExtendedSpendingKey::Master(seed);
}
...
TestSaplingNote GetTestSaplingNote(const libbitcoinlove::SaplingPaymentAddress& pa, CAmount value) {
    ...
    libbitcoinlove::SaplingNote note(pa, value);
