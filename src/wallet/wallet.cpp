...
using namespace std;
using namespace libbitcoinlove;
...
libbitcoinlove::SproutPaymentAddress CWallet::GenerateNewSproutZKey()
{
...
SaplingPaymentAddress CWallet::GenerateNewSaplingZKey()
{
    ...
    auto m = libbitcoinlove::SaplingExtendedSpendingKey::Master(seed);
    ...
    libbitcoinlove::SaplingExtendedSpendingKey xsk;
    do
    {
...
bool CWallet::AddSproutZKey(const libbitcoinlove::SproutSpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapSproutZKeyMetadata
...
bool CWallet::AddCryptedSproutSpendingKey(
    ...
    const libbitcoinlove::ReceivingKey &rk,
...
bool CWallet::AddCryptedSaplingSpendingKey(const libbitcoinlove::SaplingExtendedFullViewingKey &extfvk,
                                           const std::vector<unsigned char> &vchCryptedSecret,
                                           const libbitcoinlove::SaplingPaymentAddress &defaultAddr)
{
...
bool CWallet::LoadCryptedZKey(const libbitcoinlove::SproutPaymentAddress &addr, const libbitcoinlove::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
...
bool CWallet::LoadCryptedSaplingZKey(
    const libbitcoinlove::SaplingExtendedFullViewingKey &extfvk,
    const std::vector<unsigned char> &vchCryptedSecret)
{
     return CCryptoKeyStore::AddCryptedSaplingSpendingKey(extfvk, vchCryptedSecret, extfvk.DefaultAddress());
}

bool CWallet::LoadSaplingZKeyMetadata(const libbitcoinlove::SaplingIncomingViewingKey &ivk, const CKeyMetadata &meta)
{
    AssertLockHeld(cs_wallet); // mapSaplingZKeyMetadata
...
bool CWallet::LoadSaplingZKey(const libbitcoinlove::SaplingExtendedSpendingKey &key)
{
...
bool CWallet::LoadZKey(const libbitcoinlove::SproutSpendingKey &key)
{
    return CCryptoKeyStore::AddSproutSpendingKey(key);
}

bool CWallet::AddSproutViewingKey(const libbitcoinlove::SproutViewingKey &vk)
{
...
bool CWallet::RemoveSproutViewingKey(const libbitcoinlove::SproutViewingKey &vk)
{
...
bool CWallet::LoadSproutViewingKey(const libbitcoinlove::SproutViewingKey &vk)
{
...
std::set<std::pair<libbitcoinlove::PaymentAddress, uint256>> CWallet::GetNullifiersForAddresses(
        const std::set<libbitcoinlove::PaymentAddress> & addresses)
{
    std::set<std::pair<libbitcoinlove::PaymentAddress, uint256>> nullifierSet;
    // Sapling ivk -> list of addrs map
    // (There may be more than one diversified address for a given ivk.)
    std::map<libbitcoinlove::SaplingIncomingViewingKey, std::vector<libbitcoinlove::SaplingPaymentAddress>> ivkMap;
    for (const auto & addr : addresses) {
        auto saplingAddr = boost::get<libbitcoinlove::SaplingPaymentAddress>(&addr);
        if (saplingAddr != nullptr) {
            libbitcoinlove::SaplingIncomingViewingKey ivk;
...
bool CWallet::IsNoteSproutChange(
        const std::set<std::pair<libbitcoinlove::PaymentAddress, uint256>> & nullifierSet,
...
bool CWallet::IsNoteSaplingChange(const std::set<std::pair<libbitcoinlove::PaymentAddress, uint256>> & nullifierSet,
        const libbitcoinlove::PaymentAddress & address,
                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vJoinSplit[i].h_sig(
                            *pbitcoinloveParams, wtxItem.second.joinSplitPubKey);
                                                         ...
                                                         uint8_t n) const
{
    ...
    auto note_pt = libbitcoinlove::SproutNotePlaintext::decrypt(
        dec,
    ...
    if (note.cm() != jsdesc.commitments[n]) {
        throw libbitcoinlove::note_decryption_failed();
    }

    // SpendingKeys are only available if:
    // - We have them (this isn't a viewing key)
    // - The wallet is unlocked
    libbitcoinlove::SproutSpendingKey key;
...
mapSproutNoteData_t CWallet::FindMySproutNotes(const CTransaction &tx) const
{
    ...
    for (size_t i = 0; i < tx.vJoinSplit.size(); i++) {
        auto hSig = tx.vJoinSplit[i].h_sig(*pbitcoinloveParams, tx.joinSplitPubKey);
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (IsLockedCoin((*it).first, i) && nCoinType != ONLY_10000 && nCoinType != ONLY_25000 && nCoinType != ONLY_100000 && nCoinType != ALL_BLVNODE )
                    continue;
...
{
    std::set<libbitcoinlove::PaymentAddress> filterAddresses;
    if (address.length() > 0) {
    ...
    bool ignoreUnspendable)
{
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapSproutNoteData) {
            ...
            auto hSig = wtx.vJoinSplit[i].h_sig(*pbitcoinloveParams, wtx.joinSplitPubKey);
            try {
        ...
        for (auto & pair : wtx.mapSaplingNoteData) {
            ...
            if (requireSpendingKey) {
                libbitcoinlove::SaplingIncomingViewingKey ivk;
                libbitcoinlove::SaplingFullViewingKey fvk;
...
//
bool PaymentAddressBelongsToWallet::operator()(const libbitcoinlove::SproutPaymentAddress &zaddr) const
{
    return m_wallet->HaveSproutSpendingKey(zaddr) || m_wallet->HaveSproutViewingKey(zaddr);
}

bool PaymentAddressBelongsToWallet::operator()(const libbitcoinlove::SaplingPaymentAddress &zaddr) const
{
    libbitcoinlove::SaplingIncomingViewingKey ivk;
...
bool PaymentAddressBelongsToWallet::operator()(const libbitcoinlove::InvalidEncoding& no) const
{
    return false;
}

bool HaveSpendingKeyForPaymentAddress::operator()(const libbitcoinlove::SproutPaymentAddress &zaddr) const
{
    return m_wallet->HaveSproutSpendingKey(zaddr);
}

bool HaveSpendingKeyForPaymentAddress::operator()(const libbitcoinlove::SaplingPaymentAddress &zaddr) const
{
    libbitcoinlove::SaplingIncomingViewingKey ivk;
    libbitcoinlove::SaplingFullViewingKey fvk;
...
bool HaveSpendingKeyForPaymentAddress::operator()(const libbitcoinlove::InvalidEncoding& no) const
{
    return false;
}

boost::optional<libbitcoinlove::SpendingKey> GetSpendingKeyForPaymentAddress::operator()(
    const libbitcoinlove::SproutPaymentAddress &zaddr) const
{
    libbitcoinlove::SproutSpendingKey k;
    if (m_wallet->GetSproutSpendingKey(zaddr, k)) {
        return libbitcoinlove::SpendingKey(k);
    } else {
        return boost::none;
    }
}
...
boost::optional<libbitcoinlove::SpendingKey> GetSpendingKeyForPaymentAddress::operator()(
    const libbitcoinlove::SaplingPaymentAddress &zaddr) const
{
    libbitcoinlove::SaplingExtendedSpendingKey extsk;
    if (m_wallet->GetSaplingExtendedSpendingKey(zaddr, extsk)) {
        return libbitcoinlove::SpendingKey(extsk);
    } else {
        return boost::none;
    }
}
...
boost::optional<libbitcoinlove::SpendingKey> GetSpendingKeyForPaymentAddress::operator()(
    const libbitcoinlove::InvalidEncoding& no) const
{
    // Defaults to InvalidEncoding
    return libbitcoinlove::SpendingKey();
}

SpendingKeyAddResult AddSpendingKeyToWallet::operator()(const libbitcoinlove::SproutSpendingKey &sk) const {
...
SpendingKeyAddResult AddSpendingKeyToWallet::operator()(const libbitcoinlove::SaplingExtendedSpendingKey &sk) const {
...
SpendingKeyAddResult AddSpendingKeyToWallet::operator()(const libbitcoinlove::InvalidEncoding& no) const {
