...
{
    return MIN_PEER_PROTO_VERSION_BLVNODE;
}
...
void Payments::ProcessMessageZelnodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "znget") { //Zelnode Payments Request Sync
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION_BLVNODE) return;
...
bool PaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (pzn->protocolVersion < MIN_PEER_PROTO_VERSION_BLVNODE) {
        strError = strprintf("Zelnode protocol too old %d - req %d", pzn->protocolVersion, MIN_PEER_PROTO_VERSION_BLVNODE);
        LogPrint("zelnode","%s - %s\n", __func__, strError);
        return false;
    }

    int n = zelnodeman.GetZelnodeRank(vinZelnode, nBlockHeight - 100, MIN_PEER_PROTO_VERSION_BLVNODE);
    ...
    if (tier == Zelnode::NONE && IsSporkActive(SPORK_2_BLVNODE_UPGRADE_VOTE_ENFORCEMENT)) {
        strError = strprintf("Zelnode winner tier is %s and %s is active", TierToString(tier), sporkManager.GetSporkNameByID(SPORK_2_BLVNODE_UPGRADE_VOTE_ENFORCEMENT));
        LogPrint("zelnode","%s - %s\n", __func__, strError);
        return false;
    }
...
bool Payments::ProcessBlock(int nBlockHeight)
{
    ...
    int n = zelnodeman.GetZelnodeRank(activeZelnode.vin, nBlockHeight - 100, MIN_PEER_PROTO_VERSION_BLVNODE);
...
{
    CInv inv(MSG_BLVNODE_WINNER, GetHash());
    RelayInv(inv);
}
...
void Payments::Sync(CNode* node, int nCountNeeded)
{
    ...
    while (it != mapZelnodePayeeVotes.end()) {
        ...
        if (winner.nBlockHeight >= nHeight - nCountNeeded && winner.nBlockHeight <= nHeight + 20) {
            node->PushInventory(CInv(MSG_BLVNODE_WINNER, winner.GetHash()));
            nInvCount++;
        }
        ++it;
    }
    node->PushMessage("ssc", BLVNODE_SYNC_MNW, nInvCount);
}
