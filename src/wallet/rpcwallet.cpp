...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Bitcoinlove address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) MUST be set to the empty string \"\" to represent the default account. Passing any other string will result in an error.\n"
            "\nResult:\n"
            "\"bitcoinloveaddress\"   (string) The account Bitcoinlove address\n"
            "\nExamples:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"bitcoinloveaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            ...
            "                             The recipient will receive less Bitcoinlove than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns the total amount received by the given Bitcoinlove address in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a Bitcoinlove address.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The bitcoinlove address is the key, the numeric amount in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less Bitcoinlove than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoinlove address: ") + name_);
        }
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending Bitcoinlove\n"
            "\nArguments:\n"
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending Bitcoinlove.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoinlove address: ") + input.get_str());
            }
...
UniValue z_listunspent(const UniValue& params, bool fHelp)
{
    ...
    std::set<libbitcoinlove::PaymentAddress> zaddrs = {};
    ...
    if (params.size() > 3) {
        ...
        std::set<libbitcoinlove::SproutPaymentAddress> sproutzaddrs = {};
        pwalletMain->GetSproutPaymentAddresses(sproutzaddrs);
        
        // Sapling support
        std::set<libbitcoinlove::SaplingPaymentAddress> saplingzaddrs = {};
    ...
    if (zaddrs.size() > 0) {
        ...
        for (auto & entry : sproutEntries) {
            ...
            bool hasSproutSpendingKey = pwalletMain->HaveSproutSpendingKey(boost::get<libbitcoinlove::SproutPaymentAddress>(entry.address));
        ...
        for (auto & entry : saplingEntries) {
            ...
            pwalletMain->GetSaplingIncomingViewingKey(boost::get<libbitcoinlove::SaplingPaymentAddress>(entry.address), ivk);
...
UniValue zc_raw_receive(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libbitcoinlove::SproutSpendingKey>(&spendingkey) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout spending keys");
    }
    SproutSpendingKey k = boost::get<libbitcoinlove::SproutSpendingKey>(spendingkey);
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : outputs.getKeys()) {
        ...
        if (boost::get<libbitcoinlove::SproutPaymentAddress>(&addrTo) == nullptr) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout payment addresses");
        }
        CAmount nAmount = AmountFromValue(outputs[name_]);

        vjsout.push_back(JSOutput(boost::get<libbitcoinlove::SproutPaymentAddress>(addrTo), nAmount));
    }
    ...
    {
        auto verifier = libbitcoinlove::ProofVerifier::Strict();
        assert(jsdesc.Verify(*pbitcoinloveParams, verifier, joinSplitPubKey));
    }
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pbitcoinloveParams, joinSplitPubKey);
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"bitcoinloveaddress\"    (string) The new shielded address.\n"
            "\nExamples:\n"
...
UniValue z_listaddresses(const UniValue& params, bool fHelp)
{
    ...
    UniValue ret(UniValue::VARR);
    {
        std::set<libbitcoinlove::SproutPaymentAddress> addresses;
    ...
    {
        std::set<libbitcoinlove::SaplingPaymentAddress> addresses;
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libbitcoinlove::SproutPaymentAddress>(&zaddr) != nullptr) {
    ...
    } else if (boost::get<libbitcoinlove::SaplingPaymentAddress>(&zaddr) != nullptr) {
...
UniValue z_sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (!fromTaddr) {
        ...
        fromSapling = boost::get<libbitcoinlove::SaplingPaymentAddress>(&res) != nullptr;
    }
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        if (!IsValidDestination(taddr)) {
            ...
            if (IsValidPaymentAddress(res)) {
                ...
                bool toSapling = boost::get<libbitcoinlove::SaplingPaymentAddress>(&res) != nullptr;
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (!fEnableMergeToAddress) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: z_mergetoaddress is disabled. Run './bitcoinlove-cli help z_mergetoaddress' for instructions on how to enable this feature.");
    }
    ...
    if (!IsValidDestination(taddr)) {
        ...
        if (IsValidPaymentAddress(decodeAddr)) {
            if (boost::get<libbitcoinlove::SaplingPaymentAddress>(&decodeAddr) != nullptr) {
