...
Push the resulting branch to github:
    ...
    $ git push 'git@github.com:$YOUR_GITHUB_NAME/bitcoinlove' $(git rev-parse --abbrev-ref HEAD)
...
- Run the [Gitian deterministic build environment](https://github.com/bitcoinlove/bitcoinlove-gitian)
- Compare the uploaded [build manifests on gitian.sigs](https://github.com/bitcoinlove/gitian.sigs)
- If all is well, the DevOps engineer will build the Debian packages and update the
  [apt.zel.cash package repository](https://apt.zel.cash).

## Add release notes to GitHub

- Go to the [GitHub tags page](https://github.com/bitcoinlove/bitcoinlove/tags).
...
Notify the Bitcoinlove DevOps engineer/sysadmin that the release has been tagged. They update some variables in the company's automation code and then run an Ansible playbook, which:

* builds Bitcoinlove based on the specified branch
* deploys it as a public service (e.g. betatestnet.zel.cash, mainnet.zel.cash)
* often the same server can be re-used, and the role idempotently handles upgrades, but if not then they also need to update DNS records
* possible manual steps: blowing away the `testnet3` dir, deleting old parameters, restarting DNS seeder

Then, verify that nodes can connect to the testnet server, and update the guide on the wiki to ensure the correct hostname is listed in the recommended bitcoinlove.conf.

### Update the 1.0 User Guide

This also means updating [the translations](https://github.com/bitcoinlove/bitcoinlove-docs).
Coordinate with the translation team for now. Suggestions for improving this
part of the process should be added to #2596.

### Publish the release announcement (blog, github, bitcoinlove-dev, slack)
