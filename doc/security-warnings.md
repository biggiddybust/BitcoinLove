...
Zcash (Bitcoinlove upstream) has been subjected to a formal third-party security review. For security
...
This implementation of Bitcoinlove is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `bitcoinloved` process is running on will
be able to:
...
unprivileged) on the hardware your `bitcoinloved` process runs on until these
