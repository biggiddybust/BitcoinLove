...
#ifndef BLVCASH_PAYMENTDISCLOSURE_H
#define BLVCASH_PAYMENTDISCLOSURE_H
...
struct PaymentDisclosureInfo {
    ...
    uint256 esk;              // bitcoinlove/NoteEncryption.cpp
    uint256 joinSplitPrivKey; // primitives/transaction.h
    // ed25519 - not tied to implementation e.g. libsodium, see ed25519 rfc

    libbitcoinlove::SproutPaymentAddress zaddr;

    PaymentDisclosureInfo() : version(PAYMENT_DISCLOSURE_VERSION_EXPERIMENTAL) {
    }


    PaymentDisclosureInfo(uint8_t v, uint256 esk, uint256 key, libbitcoinlove::SproutPaymentAddress zaddr) : version(v), esk(esk), joinSplitPrivKey(key), zaddr(zaddr) { }
...
struct PaymentDisclosurePayload {
    ...
    libbitcoinlove::SproutPaymentAddress zaddr; // bitcoinlove/Address.hpp
...
#endif // BLVCASH_PAYMENTDISCLOSURE_H
