...
SpendDescriptionInfo::SpendDescriptionInfo(
    libbitcoinlove::SaplingExpandedSpendingKey expsk,
...
void TransactionBuilder::AddSaplingOutput(
    uint256 ovk,
    libbitcoinlove::SaplingPaymentAddress to,
    CAmount value,
    std::array<unsigned char, ZC_MEMO_SIZE> memo)
{
    ...
    auto note = libbitcoinlove::SaplingNote(to, value);
    outputs.emplace_back(ovk, note, memo);
    mtx.valueBalance -= value;
}

void TransactionBuilder::AddSproutInput(
    libbitcoinlove::SproutSpendingKey sk,
...
void TransactionBuilder::AddSproutOutput(
    libbitcoinlove::SproutPaymentAddress to,
    CAmount value,
    std::array<unsigned char, ZC_MEMO_SIZE> memo)
{
    if (sproutParams == nullptr) {
    ...
    libbitcoinlove::JSOutput jsOutput(to, value);
    jsOutput.memo = memo;
...
void TransactionBuilder::SendChangeTo(libbitcoinlove::SaplingPaymentAddress changeAddr, uint256 ovk)
{
...
void TransactionBuilder::SendChangeTo(libbitcoinlove::SproutPaymentAddress changeAddr)
{
...
TransactionBuilderResult TransactionBuilder::Build()
{
    ...
    if (change > 0) {
        ...
        if (saplingChangeAddr) {
            ...
            libbitcoinlove::SaplingPaymentAddress changeAddr(note.d, note.pk_d);
    ...
    for (auto output : outputs) {
        ...
        libbitcoinlove::SaplingNotePlaintext notePlaintext(output.note, output.memo);
        ...
        libbitcoinlove::SaplingOutgoingPlaintext outPlaintext(output.note.pk_d, encryptor.get_esk());
...
void TransactionBuilder::CreateJSDescriptions()
{
    ...
    std::deque<libbitcoinlove::JSInput> jsInputsDeque;
    for (auto jsInput : jsInputs) {
        jsInputsDeque.push_back(jsInput);
    }
    std::deque<libbitcoinlove::JSOutput> jsOutputsDeque;
    for (auto jsOutput : jsOutputs) {
    ...
    if (jsInputs.empty()) {
        ...
        while (jsOutputsDeque.size() > 0) {
            ...
            std::array<libbitcoinlove::JSInput, ZC_NUM_JS_INPUTS> vjsin;
            std::array<libbitcoinlove::JSOutput, ZC_NUM_JS_OUTPUTS> vjsout;
            uint64_t vpub_old = 0;
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            try {
                auto plaintext = libbitcoinlove::SproutNotePlaintext::decrypt(
                    decryptor,
                ...
                vjsin[0] = libbitcoinlove::JSInput(changeWitness.get(), note, changeKey);
        ...
        libbitcoinlove::JSOutput recipient;
        ...
        if (jsOutputsDeque.empty() && jsInputsDeque.empty()) {
            ...
            if (jsInputValue > outAmount) {
                ...
                libbitcoinlove::JSOutput recipientDue(recipient.addr, due);
        ...
        if (jsChange > 0) {
            vjsout[1] = libbitcoinlove::JSOutput(changeAddress, jsChange);
