...
PACKAGE_VERSION=$($SRC_PATH/src/bitcoinloved --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/bitcoinloved $DEB_BIN
cp $SRC_PATH/src/bitcoinlove-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/bitcoinloved.1 $DEB_MAN
cp $SRC_DOC/man/bitcoinlove-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/bitcoinloved.bash-completion $DEB_CMP/bitcoinloved
cp $SRC_PATH/contrib/bitcoinlove-cli.bash-completion $DEB_CMP/bitcoinlove-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/bitcoinloved.1
gzip --best -n $DEB_MAN/bitcoinlove-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/bitcoinloved $DEB_BIN/bitcoinlove-cli
