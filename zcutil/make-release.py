...
def main(args=sys.argv[1:]):
    """
    Perform the final Bitcoinlove release process up to the git tag.
    """
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libbitcoinloveconsensus.pc',
...
def patch_README(release, releaseprev):
    with PathPatcher('README.md') as (inf, outf):
        ...
        assert firstline == 'Bitcoinlove {}\n'.format(releaseprev.novtext), \
            repr(firstline)

        outf.write('Bitcoinlove {}\n'.format(release.novtext))
        outf.write(inf.read())
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "bitcoinlove-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "bitcoinlove-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './bitcoinlove-make-release.log'
    ...
    logging.info('bitcoinlove make-release.py debug log: %r', logname)
