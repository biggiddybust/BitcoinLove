...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libbitcoinlove's job to ensure the
    ...
    auto k = libbitcoinlove::SproutSpendingKey::random();
    auto addr = k.address();

    libbitcoinlove::SproutNote note(addr.a_pk, 100, uint256(), uint256());
    ...
    std::array<libbitcoinlove::JSInput, ZC_NUM_JS_INPUTS> inputs = {
        libbitcoinlove::JSInput(witness, note, k),
        libbitcoinlove::JSInput() // dummy input of zero value
    };
    std::array<libbitcoinlove::JSOutput, ZC_NUM_JS_OUTPUTS> outputs = {
        libbitcoinlove::JSOutput(addr, 50),
        libbitcoinlove::JSOutput(addr, 50)
    };
    ...
    {
        JSDescription jsdesc(false, *pbitcoinloveParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pbitcoinloveParams, verifier, joinSplitPubKey));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pbitcoinloveParams, verifier, joinSplitPubKey));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(false, *pbitcoinloveParams, joinSplitPubKey, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(false, *pbitcoinloveParams, joinSplitPubKey, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(false, *pbitcoinloveParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pbitcoinloveParams, verifier, joinSplitPubKey));
    }
}
