...
bool AsyncRPCOperation_saplingmigration::main_impl() {
    ...
    // If the remaining amount to be migrated is less than 0.01 BLV, end the migration.
    ...
    libbitcoinlove::SaplingPaymentAddress migrationDestAddress = getMigrationDestAddress(seed);
    ...
    do {
        ...
        auto builder = TransactionBuilder(consensusParams, targetHeight_, MIGRATION_EXPIRY_DELTA, pwalletMain, pbitcoinloveParams,
                                          &coinsView, &cs_main);
        ...
        for (const SproutNoteEntry& sproutEntry : fromNotes) {
            ...
            libbitcoinlove::SproutSpendingKey sproutSk;
            ...
            // TODO: the above functionality (in comment) is not implemented in bitcoinloved
            uint256 inputAnchor;
            std::vector<boost::optional<SproutWitness>> vInputWitnesses;
            pwalletMain->GetSproutNoteWitnesses(vOutPoints, vInputWitnesses, inputAnchor);
            builder.AddSproutInput(sproutSk, sproutEntry.note, vInputWitnesses[0].get());
        }
        // The amount chosen *includes* the 0.0001 BLV fee for this transaction, i.e.
        // the value of the Sapling output will be 0.0001 BLV less.
        builder.SetFee(FEE);
...
libbitcoinlove::SaplingPaymentAddress AsyncRPCOperation_saplingmigration::getMigrationDestAddress(const HDSeed& seed) {
    if (mapArgs.count("-migrationdestaddress")) {
        ...
        auto saplingAddress = boost::get<libbitcoinlove::SaplingPaymentAddress>(&address);
    ...
    libbitcoinlove::SaplingExtendedSpendingKey xsk = m_32h_cth.Derive(0 | ZIP32_HARDENED_KEY_LIMIT);

    libbitcoinlove::SaplingPaymentAddress toAddress = xsk.DefaultAddress();
